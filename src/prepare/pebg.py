...
with tf.Session() as sess:
    ...
    if model_flag == 0:
        ...
        saver.restore(sess, os.path.join(saved_model_folder, 'pebg_t%d.ckpt'%model_flag))
    ...
    for i in range(model_flag, epochs):
        ...
        if i+1 in [20,50,60, 100,500]: 
            saver.save(sess, os.path.join(saved_model_folder, 'pebg_t%d.ckpt'%(i+1)))
      
        #wandb.save(saved_model_folder)
    ...
    np.savez(os.path.join(data_folder_output, 'embedding_t%d.npz'%epochs),
